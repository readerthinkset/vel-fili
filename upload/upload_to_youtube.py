"""
YouTube Upload Script - VELOCITY FILIPINO
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Filipino Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Filipino with VELOCITY FILIPINO!", "", f"Category: {category}", "", f"Master Filipino one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('filipino', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily Filipino lessons!", "", f"#LearnFilipino #FilipinoLessons #FilipinoForBeginners #LanguageLearning", f"#Filipino #Education #Tutorial #DailyFilipino #{category.replace(' ', '')}", f"#VELOCITYFILIPINO #FilipinoPhrases #SpeakFilipino"])
    tags = [f"learn filipino", f"filipino lessons", f"filipino for beginners", f"filipino phrases", "language learning", f"speak filipino", category.lower(), "education", f"daily filipino", "velocity filipino", f"filipino learning"]
    return title, "\n".join(lines), tags

def upload_to_youtube(video_path, title, description, tags=None, category_id='22'):
    if tags is None: tags = ['education', 'language learning', '#Filipino']
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Filipino Daily", description=f"#shorts #learnfilipino #filipino #language #education", tags=['#Filipino', 'education', 'language learning'], category_id='22')
